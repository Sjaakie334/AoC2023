...
function getcalibrationValue () {
	...
	caliArr.forEach(value => {
		const resultArray = value.match(/one|two|three|four|five|six|seven|eight|nine|zero|\d/g) || [];
		const firstEntry = resultArray[0];
		const lastEntry = resultArray[resultArray.length - 1];

		const firstValue = toNumber(firstEntry);
		const lastValue = toNumber(lastEntry);
...
function getResult () {
	...
	getcalibrationValue().forEach(valueString => {
